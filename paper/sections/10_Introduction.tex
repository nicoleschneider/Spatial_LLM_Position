...
However, by extending the coverage of spatial reasoning tasks to include \nrscomment{aspects that they missed that we show LLMs do badly}, we find that \nrscomment{conclusion}.


\nrscomment{describe the tests and motivate each one}

\nrscomment{describe findings of each}
...
\nrscomment{update this list}

\nrscomment{describe significance of findings}

\nrscomment{update FW suggestions}
...
Once embedding methods have been well-established for geodata, it will then be possible to leverage recent and future work in multimodal learning~\cite{Xue2023, Trappolini2023} and symbolic reasoning \nrscomment{need a cite} to combine different input modalities and train a geo-foundation model using self-supervised training objectives that will generalize to a variety of downstream spatial tasks.

\nrscomment{how will it address the shortcomings we identify through our experiments}
