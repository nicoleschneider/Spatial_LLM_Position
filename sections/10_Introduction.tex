...
Spatial information is ubiquitous, and it can come in a variety of forms, including in text, making it prevalent in the data used to train LLMs.
...
However, LLMs are still a long way from inferring implicit spatial knowledge from geospatial information, which limits their ability to perform tasks that have a spatial component, such as generating a set of instructions for how to navigate from one place to another, or recommending points of interest based on spatial constraints.

With these use cases in mind, we designed a set of experiments to probe an LLM to determine if it is capable of spatial reasoning, or if it just memorized some spatial facts from training.
%
...
%
To address these shortcomings, we envision new embedding methods and self-supervised learning objectives that can be used to enable LLMs to learn over complex spatial data.
%
